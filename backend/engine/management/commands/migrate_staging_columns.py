...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.migrate_fk_model(LinkHubSourceMapping, 'staging_column', ['source_column', 'prejoin_extraction_column'])
    ...
    def migrate_fk_model(self, model_class, staging_field_name, legacy_field_names):
        ...
        for mapping in mappings:
            ...
            if legacy_col:
                setattr(mapping, staging_field_name, get_or_create_staging_column(legacy_col))
                mapping.save()
                count += 1
                
        self.stdout.write(self.style.SUCCESS(f'Successfully migrated {count} {model_class.__name__} records.'))
