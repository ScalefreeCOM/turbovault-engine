...
class DbtProjectGenerator:
    ...
    def _generate_sources(self, sources: list) -> None:
        ...
        if template:
            ...
            path = self.folder_config.get_staging_base_path(self.output_path) / "sources.yml"
            write_yaml_file(path, content)
    ...
    def _generate_hubs(self, hubs: list[HubDefinition]) -> None:
        ...
        for hub in hubs:
            try:
                ...
                sql_template, yaml_template = self.template_resolver.get_templates(entity_type)
        ...
        for satellite in satellites:
            try:
                ...
                if generate_v1 and satellite.satellite_type in v1_types:
                    self._generate_satellite_v1(satellite, output_dir, v0_name, v1_suffix)
                
            except Exception as e:
                logger.warning(f"Failed to generate satellite {satellite.satellite_name}: {e}")
                self.report.add_error(
    ...
    def _generate_satellite_v1(
        ...
        sql_template, yaml_template = self.template_resolver.get_templates("satellite_v1")
    ...
    def _generate_pits(self, pits: list[PITDefinition]) -> None:
        ...
        for pit in pits:
            try:
                output_dir = self.folder_config.get_business_vault_pits_path(self.output_path)
    ...
    def _generate_reference_tables(self, ref_tables: list[ReferenceTableDefinition]) -> None:
        """Generate reference table SQL and YAML models."""
        if not ref_tables:
            logger.debug("No reference tables to generate")
            return
        
        sql_template, yaml_template = self.template_resolver.get_templates("reference_table")
        ...
        for ref_table in ref_tables:
            try:
                output_dir = self.folder_config.get_business_vault_reference_tables_path(
                    self.output_path
                )
                ...
                path = output_dir / filename
                write_sql_file(path, content)
                self.report.add_file(path, "reference_table", ref_table.table_name, "sql")
                ...
                if yaml_template:
                    ...
                    path = output_dir / filename
                    write_yaml_file(path, content)
                    self.report.add_file(path, "reference_table", ref_table.table_name, "yaml")
                
                self.report.reference_tables_generated += 1
                
            except Exception as e:
                logger.warning(f"Failed to generate reference table {ref_table.table_name}: {e}")
                self.report.add_error(
                    entity_type="reference_table",
    ...
    def _generate_snapshot_controls(
        self,
        snapshot_controls: list[SnapshotControlDefinition]
    ) -> None:
        ...
        for snap_ctrl in snapshot_controls:
            try:
                ...
                if v0_sql_template:
                    ...
                    path = output_dir / filename
                    write_sql_file(path, content)
                    self.report.add_file(path, "snapshot_control_v0", snap_ctrl.v0_name, "sql")
                    
                    if v0_yaml_template:
                        content = v0_yaml_template.render(**snap_ctrl.model_dump())
                        filename = get_model_filename(snap_ctrl.v0_name, extension="yml")
                        path = output_dir / filename
                        write_yaml_file(path, content)
                        self.report.add_file(path, "snapshot_control_v0", snap_ctrl.v0_name, "yaml")
                ...
                if v1_sql_template:
                    ...
                    path = output_dir / filename
                    write_sql_file(path, content)
                    self.report.add_file(path, "snapshot_control_v1", snap_ctrl.v1_name, "sql")
                    
                    if v1_yaml_template:
                        content = v1_yaml_template.render(**snap_ctrl.model_dump())
                        filename = get_model_filename(snap_ctrl.v1_name, extension="yml")
                        path = output_dir / filename
                        write_yaml_file(path, content)
                        self.report.add_file(path, "snapshot_control_v1", snap_ctrl.v1_name, "yaml")
                
                self.report.snapshot_controls_generated += 1
                
            except Exception as e:
                logger.warning(f"Failed to generate snapshot control {snap_ctrl.name}: {e}")
                self.report.add_error(
