...
class ExcelSourceConfig(BaseModel):
    ...
    type: Literal[SourceType.EXCEL] = Field(
        SourceType.EXCEL,
        description="Source type (must be 'excel')"
    )
    path: Path = Field(
        ...,
        description="Path to Excel file containing source metadata"
    )
    ...
    def validate_path_exists(cls, v: Path) -> Path:
        ...
        if not v.exists():
            import warnings
            warnings.warn(f"Excel file not found: {v}. It will need to exist before import.")
        return v
...
class ProjectConfiguration(BaseModel):
    ...
    stage_schema: str = Field(
        "stage",
        description="Schema name for staging layer"
    )
    stage_database: Optional[str] = Field(
        None,
        description="Optional database name for staging layer"
    )
    rdv_schema: str = Field(
        "rdv",
        description="Schema name for Raw Data Vault layer"
    )
    rdv_database: Optional[str] = Field(
        None,
        description="Optional database name for Raw Data Vault layer"
    )
    hashdiff_naming_pattern: Optional[str] = Field(
        None,
        description="Optional naming pattern for hashdiff columns (future use)"
    )
...
class OutputConfiguration(BaseModel):
    ...
    dbt_project_name: Optional[str] = Field(
        None,
        description="Name of the dbt project (defaults to project.name)"
    )
...
class TurboVaultConfig(BaseModel):
    ...
    source: Optional[ExcelSourceConfig] = Field(
        None,
        description="Optional source metadata import configuration"
    )
