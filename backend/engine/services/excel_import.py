...
class ExcelImportService:
    ...
    def __init__(self, file_path: str):
        ...
        self._prejoins: Dict[str, PrejoinDefinition] = {} # key: source_table_id|target_table_id
        self._extractions: Dict[str, PrejoinExtractionColumn] = {} # key: prejoin_id|source_col_name
        self._snapshot_control: Optional[SnapshotControlTable] = None
        self._snapshot_logic: Optional[SnapshotControlLogic] = None

    @transaction.atomic
    def import_metadata(self, project_name: str, description: Optional[str] = None) -> Project:
        """
        ...
        sat_sheets = ["standard_satellite", "ref_sat", "non_historized_satellite", "multiactive_satellite"]
    ...
    def _collect_all_source_columns(self):
        ...
        for table_key, cols in columns_to_create.items():
            ...
            if not table:
                logger.warning(f"Source table {table_key} not found in source_data, but referenced columns exist.")
                continue
    ...
    def _process_standard_hubs(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            if not hub_name or hub_name == "nan": continue
        ...
        for hub in Hub.objects.filter(project=self.project):
            for col in hub.columns.all():
                if not HubSourceMapping.objects.filter(hub_column=col, is_primary_source=True).exists():
                    first_mapping = HubSourceMapping.objects.filter(hub_column=col).first()
                    if first_mapping:
    ...
    def _process_reference_hubs(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            source_col_name = str(row.get("source_column_physical_name"))
            if not source_col_name or source_col_name == "nan": continue
    ...
    def _process_standard_links(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            if not link_name or link_name == "nan": continue
            ...
            if hub_id_str:
                ...
                if not hub:
                    hub = Hub.objects.filter(project=self.project, hub_physical_name=hub_id_str).first()
                ...
                if hub and hub.hub_type == Hub.HubType.STANDARD:
                    ...
                    if col_name:
                        lc, _ = LinkColumn.objects.get_or_create(
                            link=link,
                            column_name=col_name,
                            defaults={"column_type": LinkColumn.ColumnType.BUSINESS_KEY}
                        )
    ...
    def _process_non_historized_links(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            col_name = self._get_val(row, "prejoin_target_column_alias") or self._get_val(row, "prejoin_extraction_column_name") or self._get_val(row, "source_column_physical_name")
            if col_name:
                ...
                extraction_key = f"{link.link_physical_name}|{alias or ext_col or col_name}"
                extraction = self._extractions.get(extraction_key)
    ...
    def _process_satellites(self, df: pd.DataFrame, sheet_type: str):
        ...
        for _, row in df.iterrows():
            sat_name_col = "target_reference_table_physical_name" if sheet_type == "ref_sat" else "target_satellite_table_physical_name"
            ...
            if sat_name not in self._satellites:
                ...
                parent_id = self._get_val(row, "parent_identifier") or \
                            self._get_val(row, "parent_table_identifier") or \
                            self._get_val(row, "nh_link_identifier") or \
                            self._get_val(row, "referenced_hub") or \
                            self._get_val(row, "parent_hub")
                ...
                if not parent_hub and not parent_link:
                    parent_hub = Hub.objects.filter(project=self.project, hub_physical_name=parent_id).first()
                    parent_link = Link.objects.filter(project=self.project, link_physical_name=parent_id).first()
                ...
                if (parent_hub or parent_link) and source_table:
                    ...
                    sat_id_col = "ma_satellite_identifier" if sheet_type == "multiactive_satellite" else \
                                 "reference_satellite_identifier" if sheet_type == "ref_sat" else \
                                 "satellite_identifier" # standard and non-historized often use this
                    sat_id = self._get_val(row, sat_id_col)
                    if sat_id:
                        self._satellites[sat_id] = satellite
                else:
                    logger.warning(f"Could not find parent {parent_id} or source table {source_table_key} for satellite {sat_name}")
                    continue
                ...
                if source_col:
                    ...
                    SatelliteColumn.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            "include_in_delta_detection": sheet_type != "non_historized_satellite",
        ...
        for _, row in df.iterrows():
            ...
            if prejoin_key not in self._prejoins:
                ...
                if source_table and target_table:
                    ...
                    source_col = self._source_columns.get(f"{source_table_id}|{source_col_name}")
                    target_col = self._source_columns.get(f"{target_table_id}|{target_col_name}")
                    
                    if source_col: prejoin.prejoin_condition_source_column.add(source_col)
                    if target_col: prejoin.prejoin_condition_target_column.add(target_col)
                    
                    self._prejoins[prejoin_key] = prejoin
            
            prejoin = self._prejoins.get(prejoin_key)
            if not prejoin: continue
            
            # Extraction Column
            extraction_col_name = self._get_val(row, "prejoin_extraction_column_name")
            if extraction_col_name:
                source_col = self._source_columns.get(f"{target_table_id}|{extraction_col_name}")
                if source_col:
                    ...
                    alias = self._get_val(row, "prejoin_target_column_alias")
                    self._extractions[f"{link_name}|{alias or extraction_col_name}"] = extraction
    ...
    def _process_reference_tables(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            if not ref_table_name: continue

            hub = self._hubs.get(hub_id)
            if not hub:
                hub = Hub.objects.filter(project=self.project, hub_physical_name=hub_id).first()
            
            if not hub:
                logger.warning(f"Hub {hub_id} not found for reference table {ref_table_name}")
                continue
            ...
            ref_table, _ = ReferenceTable.objects.get_or_create(
                ...
                defaults={
                    "reference_hub": hub,
                    "historization_type": hist_type_map.get(hist_val.upper(), ReferenceTable.HistorizationType.LATEST)
                }
            )
            ...
            if not sat:
                sat = Satellite.objects.filter(project=self.project, satellite_physical_name=sat_id).first()
            ...
            if sat:
                ...
                if include_str:
                    for col_name in include_str.split(","):
                        col = SatelliteColumn.objects.filter(satellite=sat, target_column_name=col_name.strip()).first()
                        if col: assignment.include_columns.add(col)
                
                if exclude_str:
                    for col_name in exclude_str.split(","):
                        col = SatelliteColumn.objects.filter(satellite=sat, target_column_name=col_name.strip()).first()
                        if col: assignment.exclude_columns.add(col)
    ...
    def _process_pits(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            if not hub and not link:
                hub = Hub.objects.filter(project=self.project, hub_physical_name=entity_id).first()
                link = Link.objects.filter(project=self.project, link_physical_name=entity_id).first()
            
            if not hub and not link:
                logger.warning(f"Tracked entity {entity_id} not found for PIT {pit_name}")
                continue
            ...
            if sat_ids and sat_ids != "nan":
                ...
                for sid in sat_ids.replace(";", ",").split(","):
                    ...
                    if not sat:
                        sat = Satellite.objects.filter(project=self.project, satellite_physical_name=sid).first()
                    if not sat:
                        # Try searching for satellite where physical name is sid (identifier in excel sometimes refers to physical name)
                        sat = Satellite.objects.filter(project=self.project, satellite_physical_name__iexact=sid).first()
    ...
    def _create_default_snapshot_control(self):
        ...
        if self._snapshot_control: return # Already created
        ...
        self._snapshot_control = SnapshotControlTable.objects.create(
            ...
            daily_snapshot_time=timezone.make_aware(datetime(2000, 1, 1, 8, 0, 0)).time()
        )
