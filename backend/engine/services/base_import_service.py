...
class BaseImportService:
    ...
    def import_metadata(
        ...
        if "standard_link" in sheet_names:
            self._process_prejoins(
                self.source.get_data("standard_link"), "standard_link"
            )
        if "non_historized_link" in sheet_names:
            self._process_prejoins(
                self.source.get_data("non_historized_link"), "non_historized_link"
            )
        ...
        if "non_historized_link" in sheet_names:
            self._process_non_historized_links(
                self.source.get_data("non_historized_link")
            )
    ...
    def _process_standard_hubs(self, df: pd.DataFrame):
        for _, row in df.iterrows():
            ...
            col_name = self._get_val(
                row, "business_key_physical_name"
        ...
        if "hub_identifier" in link_rows.columns:
            ...
            for hub_id, hub_rows in ref_rows.groupby("hub_identifier"):
                ...
                if not hub:
                    continue
                alias = (
                    self._get_val(hub_rows.iloc[0], "target_column_physical_name") or ""
                )
                hk_col = self._get_val(
                    hub_rows.iloc[0], "target_primary_key_physical_name"
                )
                final_alias = alias if alias != hk_col else ""
                lhr = LinkHubReference.objects.create(
                    link=link, hub=hub, hub_hashkey_alias_in_link=final_alias
                )
                hub_cols = list(
                    hub.columns.filter(
                        column_type__in=[
                ...
                for idx, (_, row) in enumerate(
                    hub_rows.sort_values("target_column_sort_order").iterrows()
                ):
                    if idx >= len(hub_cols):
                        continue
                    ...
                    prejoin_ext = self._extractions.get(
                        f"{link.link_physical_name}|{prejoin_alias or ext_col_name or source_col_name}"
                    )
                    source_col = (
                        self._source_columns.get(f"{source_table_id}|{source_col_name}")
                        if not prejoin_ext
                        else None
                    )
    ...
    def _process_satellites(self, df: pd.DataFrame, sheet_type: str):
        ...
        for sat_name, sat_rows in df.groupby(name_col):
            ...
            if sat_name not in self._satellites:
                parent_id = (
                    self._get_val(row_sample, "parent_identifier")
                    or self._get_val(row_sample, "parent_table_identifier")
                    or self._get_val(row_sample, "referenced_hub")
                )
                parent_hub = self._hubs.get(parent_id)
                parent_link = self._links.get(parent_id)
                source_table = self._source_tables.get(
                    self._get_val(row_sample, "source_table_identifier")
                )
    ...
    def _process_prejoins(self, df: pd.DataFrame, sheet_name: str):
        ...
        for _, row in df.iterrows():
            ...
            if not source_table or not target_table:
                continue
            prejoin, _ = PrejoinDefinition.objects.get_or_create(
                project=self.project,
                source_table=source_table,
            ...
            if ext_name:
                ...
                if source_col:
                    ext, _ = PrejoinExtractionColumn.objects.get_or_create(
                        prejoin=prejoin,
                        source_column=source_col,
                        defaults={
                            "prejoin_target_column_alias": self._get_val(
                                row, "prejoin_target_column_alias"
                            )
                        },
                    )
                    self._extractions[
                        f"{self._get_val(row, 'target_link_table_physical_name')}|{self._get_val(row, 'prejoin_target_column_alias') or ext_name}"
                    ] = ext
        ...
        for _, row in df.iterrows():
            ...
            if name and hub:
                ref_table, _ = ReferenceTable.objects.get_or_create(
                    project=self.project,
                    reference_table_physical_name=name,
                    defaults={"reference_hub": hub},
                )
    ...
    def _create_default_snapshot_control(self, skip_creation: bool = False):
        ...
        scl, _ = SnapshotControlLogic.objects.get_or_create(
            snapshot_control_table=sct,
            snapshot_control_logic_column_name="daily",
            defaults={
