...
class ReferenceTable(models.Model):
    ...
    reference_table_physical_name = models.CharField(
        max_length=255,
        help_text="Physical name of the reference table"
    )
...
class ReferenceTableSatelliteAssignment(models.Model):
    ...
    def clean(self) -> None:
        ...
        if self.reference_satellite and self.reference_satellite.satellite_type != Satellite.SatelliteType.REFERENCE:
            raise ValidationError({
        ...
        if (self.reference_table and self.reference_satellite and 
            self.reference_satellite.parent_hub_id != self.reference_table.reference_hub_id):
            raise ValidationError({
