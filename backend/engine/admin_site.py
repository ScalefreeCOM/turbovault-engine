...
def custom_get_app_list(self: admin.AdminSite, request: Any, app_label: str | None = None) -> list[dict[str, Any]]:
    """
    ...
    base_metadata_models = {
        'project', 'group', 'sourcesystem', 'sourcetable', 'sourcecolumn',
        'snapshotcontroltable', 'snapshotcontrollogic'
    }
    ...
    dbt_metadata_models = {
        'templatecategory', 'modeltemplate'
    }
    ...
    # Base Metadata group
    base_models = [
        model for model in engine_models
    ...
    custom_groups = [app for app in custom_apps if app.get('app_label') in custom_group_names]
    other_apps = [app for app in custom_apps if app.get('app_label') not in custom_group_names]
