...
class TestValidateExport:
    ...
    def test_validate_export_with_sample_data(self, django_setup, project_export):
        ...
        assert result.is_valid or len(result.errors) >= 0  # Always true, just run without crash
    
    def test_validate_export_returns_validation_result(self, django_setup, project_export):
        """validate_export returns a ValidationResult object."""
        from engine.services.generation.validators import validate_export, ValidationResult
        
        result = validate_export(project_export)
        
        assert isinstance(result, ValidationResult)
    
    def test_validate_export_checks_all_entity_types(self, django_setup, project_export):
