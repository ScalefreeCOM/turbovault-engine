...
This document describes the **logical domain model** for the TurboVault Engine.  
It is the blueprint for the Django ORM models and is intended to remain stable so that:
...
- **identifier**  
  Logical primary key type (e.g. `UUID`, `BIGINT`, or similar). In Django this will typically be a `UUIDField` or `BigAutoField`.

- **string**  
  Text field (e.g. `CharField` / `TextField` depending on length).
...
- **list[...]**  
  Represents a multi-value relationship. In relational / Django terms this will be implemented as:
