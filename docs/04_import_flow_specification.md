...
| | `link_type` | `'standard'` |
| | `hub_references` | List distinct `hub_identifier` values from `standard_link` |
| | `created_at` / `updated_at` | Current timestamp |
...
| | `hub_references` | List distinct `hub_identifier` values form `non_historized_link` |
| | `created_at` / `updated_at` | Current timestamp |

## 9. Link Column
**Scope:** Non-Historized Links (Standard links do not support add'l columns in old format)

| Column | Import Logic |
| :--- | :--- |
| `link_column_id` | Auto-generated UUID |
| `link_id` | Use `link_id` from Link |
| `column_name` | Logic: `COALESCE(prejoin_target_column_alias, prejoin_extraction_column_name, source_column_physical_name)` from `non_historized_link` (where `hub_identifier` is not empty) |
| `column_type` | `'payload'` |
| `sort_order` | Natural order from input file (1-based increment per link) |
| `created_at` / `updated_at` | Current timestamp |

## 10. Link Source Mapping
**Scope:** Non-Historized Links

| Column | Import Logic |
| :--- | :--- |
| `link_source_mapping_id` | Auto-generated UUID |
| `link_column_id` | Use `link_column_id` from Link Column |
| `source_column_id` | Join `source_column` on `source_column_physical_name` from `non_historized_link` |
| `created_at` / `updated_at` | Current timestamp |
